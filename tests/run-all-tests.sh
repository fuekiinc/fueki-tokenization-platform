...
run_step "Collect coverage artifact" bash -c 'if [ -f coverage/coverage-final.json ]; then cp coverage/coverage-final.json tests/reports/coverage-final.json; else echo "coverage/coverage-final.json not found"; exit 1; fi'
...
run_step "Backend tests" npm --prefix backend run test -- --reporter=default --reporter=json --outputFile=../tests/reports/backend-vitest-results.json
