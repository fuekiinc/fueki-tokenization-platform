...
export const SecurityTokenFactoryABI = [
  ...
  // Functions
  // Newer deployments: caller provides child bytecodes.
  ...
  // Legacy deployments: factory uses immutable deployer and no bytecode params.
  'function createSecurityToken(string _name, string _symbol, uint8 _decimals, uint256 _totalSupply, uint256 _maxTotalSupply, bytes32 _documentHash, string _documentType, uint256 _originalValue, uint256 _minTimelockAmount, uint256 _maxReleaseDelay) returns (address tokenAddress, address rulesAddress)',
